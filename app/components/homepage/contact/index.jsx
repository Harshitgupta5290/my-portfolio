"use client";
// @flow strict
import { personalData } from '@/utils/data/personal-data';
import { useLocale } from "@/app/context/locale-context";
import Link from 'next/link';
import SectionHeading from "../../helper/section-heading";
import { BiLogoLinkedin } from "react-icons/bi";
import { BsInstagram } from "react-icons/bs";
import { CiLocationOn } from "react-icons/ci";
import { IoLogoGithub, IoMdCall } from "react-icons/io";
import { MdAlternateEmail } from "react-icons/md";

function ContactSection() {
  const { t } = useLocale();
  return (
    <div id="contact" className="relative z-50 border-t my-12 lg:my-24 border-[var(--line)]">
      {/* Top gradient line */}
      <div className="flex justify-center -translate-y-[1px]">
        <div className="w-3/4">
          <div className="h-[1px] bg-gradient-to-r from-transparent via-violet-500 to-transparent w-full" />
        </div>
      </div>

      {/* Section header */}
      <SectionHeading
        eyebrow="Get in touch"
        gradient={t("contact.letsConnect")}
        subtitle={t("contact.subtitle")}
        className="mt-12 mb-14"
      />

      {/* Main content: info + terminal */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 lg:gap-8">

        {/* Contact info + socials — left 2 cols */}
        <div className="lg:col-span-2 flex flex-col gap-4">

          {[
            { icon: MdAlternateEmail, label: t("contact.emailLabel"), value: personalData.email, href: `mailto:${personalData.email}`, color: "#16f2b3" },
            { icon: IoMdCall, label: t("contact.phoneLabel"), value: personalData.phone, href: `tel:${personalData.phone}`, color: "#a78bfa" },
            { icon: CiLocationOn, label: t("contact.locationLabel"), value: personalData.address, href: null, color: "#ec4899" },
          ].map(({ icon: Icon, label, value, href, color }) => (
            <div
              key={label}
              className="group flex items-center gap-4 p-4 rounded-xl border border-[var(--line)] bg-[var(--card)] hover:border-[var(--ink-3)] transition-all duration-300"
            >
              <div
                className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0"
                style={{ backgroundColor: `${color}12`, border: `1px solid ${color}28` }}
              >
                <Icon size={18} style={{ color }} />
              </div>
              <div className="min-w-0">
                <p className="text-[10px] text-[var(--ink-3)] uppercase tracking-widest mb-0.5">{label}</p>
                {href ? (
                  <a href={href} className="text-[var(--ink-2)] text-sm hover:text-[var(--ink)] transition-colors duration-200 truncate block">
                    {value}
                  </a>
                ) : (
                  <p className="text-[var(--ink-2)] text-sm">{value}</p>
                )}
              </div>
            </div>
          ))}

          {/* Social links */}
          <div className="mt-2">
            <p className="text-[10px] text-[var(--ink-3)] uppercase tracking-widest mb-3">{t("contact.findMeOn")}</p>
            <div className="flex items-center gap-3 flex-wrap">
              {[
                { href: personalData.github,    Icon: IoLogoGithub,   label: "GitHub"    },
                { href: personalData.linkedIn,  Icon: BiLogoLinkedin, label: "LinkedIn"  },
                { href: personalData.instagram, Icon: BsInstagram,    label: "Instagram" },
              ].map(({ href, Icon, label }) => (
                <Link
                  key={label}
                  href={href}
                  target="_blank"
                  className="group flex items-center gap-2 px-4 py-2.5 rounded-lg border border-[var(--line)] bg-[var(--card)] hover:border-[#16f2b3]/40 hover:bg-[#16f2b305] transition-all duration-300"
                >
                  <Icon size={15} className="text-[var(--ink-3)] group-hover:text-[#16f2b3] transition-colors duration-300" />
                  <span className="text-[var(--ink-3)] group-hover:text-[var(--ink-2)] text-xs font-mono transition-colors duration-300">{label}</span>
                </Link>
              ))}
            </div>
          </div>
        </div>

        {/* Terminal — right 3 cols */}
        <div className="lg:col-span-3 rounded-xl border border-[var(--line)] bg-[var(--surface)] overflow-hidden flex flex-col">
          {/* Title bar */}
          <div className="flex items-center gap-2 px-4 py-3 border-b border-[var(--line)] flex-shrink-0">
            <div className="flex gap-1.5">
              <div className="w-3 h-3 rounded-full bg-red-500/70" />
              <div className="w-3 h-3 rounded-full bg-yellow-500/70" />
              <div className="w-3 h-3 rounded-full bg-green-500/70" />
            </div>
            <span className="text-[var(--ink-3)] text-[10px] font-mono ml-2">harshit@portfolio ~ status.sh</span>
            <div className="ml-auto flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />
              <span className="text-green-400 text-[9px] font-mono">live</span>
            </div>
          </div>

          {/* Terminal body */}
          <div className="p-6 font-mono text-xs flex-1 space-y-3">
            <div>
              <span className="text-[#16f2b3]">$</span>
              <span className="text-[var(--ink-3)]"> whoami</span>
            </div>
            <p className="text-[var(--ink-2)] pl-2">Harshit Gupta — Full Stack & AI Engineer</p>

            <div className="pt-1">
              <span className="text-[#16f2b3]">$</span>
              <span className="text-[var(--ink-3)]"> check-availability --verbose</span>
            </div>
            <div className="pl-2 space-y-1.5">
              <p className="text-[var(--ink-2)]">
                {'>'} Status: <span className="text-green-400 font-semibold">{t("contact.available")}</span> for new opportunities
              </p>
              <p className="text-[var(--ink-2)]">
                {'>'} Response time: <span className="text-[#16f2b3]">{t("contact.responseTime")}</span>
              </p>
              <p className="text-[var(--ink-2)]">
                {'>'} Timezone: <span className="text-violet-400">IST (UTC+5:30)</span>
              </p>
              <p className="text-[var(--ink-2)]">
                {'>'} Open to: <span className="text-[var(--ink)]">Full-time · Freelance · Contract</span>
              </p>
              <p className="text-[var(--ink-2)]">
                {'>'} Domains: <span className="text-[#16f2b3]">Backend · AI/ML · SaaS · Microservices</span>
              </p>
            </div>

            <div className="pt-1">
              <span className="text-[#16f2b3]">$</span>
              <span className="text-[var(--ink-3)]"> cat preferred-contact.txt</span>
            </div>
            <div className="pl-2 space-y-1.5">
              <p className="text-[var(--ink-2)]">
                {'>'} Email: <a href={`mailto:${personalData.email}`} className="text-[#16f2b3] hover:underline">{personalData.email}</a>
              </p>
              <p className="text-[var(--ink-2)]">
                {'>'} LinkedIn: <a href={personalData.linkedIn} target="_blank" className="text-violet-400 hover:underline">linkedin.com/in/harshitgupta1215</a>
              </p>
            </div>

            <div className="pt-1 flex items-center">
              <span className="text-[#16f2b3]">$</span>
              <span className="inline-block w-2 h-3.5 bg-[#16f2b3]/60 ml-1 animate-pulse" />
            </div>
          </div>

          {/* CTA strip */}
          <div className="border-t border-[var(--line)] px-6 py-4 flex items-center justify-between">
            <span className="text-[var(--ink-3)] text-[10px] font-mono">{t("contact.readyToCollaborate")}</span>
            <a
              href={`mailto:${personalData.email}`}
              className="flex items-center gap-2 px-5 py-2 rounded-lg bg-gradient-to-r from-violet-600 to-[#16f2b3]/80 text-white text-xs font-semibold uppercase tracking-wider hover:shadow-[0_0_20px_rgba(22,242,179,0.2)] transition-all duration-300"
            >
              <MdAlternateEmail size={14} />
              {t("contact.sendEmail")}
            </a>
          </div>
        </div>

      </div>
    </div>
  );
}

export default ContactSection;
